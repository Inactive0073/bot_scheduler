...
@broker.task(task_name="delete_old_or_empty_bonus", schedule=[{"cron": "* 4 * * *"}]) # запуск ежедневно в 4:00
    ...
    async with AsyncSession(engine) as session:
        try:
            ...
            stmt = delete(Bonus).where(
                and_(
                    or_(Bonus.amount == 0, Bonus.expire_date < current_time)
                )
            )
...
@broker.task(task_name="upgrade_bonus_level", schedule=[{"cron": "10 4 * * *"}]) # запуск ежедневно в 4:10
...
@broker.task(task_name="reset_visits_per_year", schedule=[{"cron": "0 0 1 1 *"}])  # Запускается 1 января в 00:00 UTC
